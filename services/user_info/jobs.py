...
def fetch_jobs(user_id, limit=None, start=None, end=None):
    jobs = []
    # it looks at z accounting database which stores historical job records.
    cmd = [
        "sacct",
        "-u", user_id,
    ...
    try:
        ...
        for line in lines:
            ...
            jobs.append(job)
            jobs.reverse() # to show the currnet job on top and older in the bottom
        # apply job limit if requested
        if limit:
            jobs = jobs[:limit]
